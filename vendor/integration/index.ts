  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork('restfulrites');

        const virtualModuleId = 'restfulrites:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-restfulrites-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === 'string') {
          ...
          buildLogger.info(`RestfulRites \`${_themeConfig}\` has been loaded.`);
        } else {
          buildLogger.info(`RestfulRites config has been loaded.`);
        }
      },
